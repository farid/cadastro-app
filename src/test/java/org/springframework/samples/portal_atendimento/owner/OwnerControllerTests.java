...
class OwnerControllerTests {
	 ...
	 *
	 * // @BeforeEach void setup() { george = new Owner(); george.setId(TEST_OWNER_ID);
	 * george.setNome("George"); george.setCpf("01010010102");
	 * george.setEndereco("110 W. Liberty St."); george.setCep("87878786");
	 * george.setTelefone("6085551023"); Pet max = new Pet(); PetType dog = new PetType();
	 * dog.setName("dog"); max.setId(1); max.setType(dog); max.setName("Max");
	 ...
	 * Visit(); visit.setDate(LocalDate.now());
	 * given(this.visits.findByPetId(max.getId())).willReturn(Collections.singletonList(
	 * visit)); }
	 *
	 ...
	 * "London").param("telephone", "01316761638"))
	 ...
	 * Owner()));
	 * mockMvc.perform(get("/owners")).andExpect(status().isOk()).andExpect(view().name(
	 * "owners/ownersList")); }
	 *
	 * // @Test void testProcessFindFormByLastName() throws Exception {
	 * given(this.owners.findByLastName(george.getNome())).willReturn(Lists.newArrayList(
	 * george)); mockMvc.perform(get("/owners").param("lastName",
	 ...
	 * .andExpect(model().attributeExists("owner")) .andExpect(model().attribute("owner",
	 ...
	 * "Joe") .param("lastName", "Bloggs").param("address",
	 ...
	 * mockMvc.perform(post("/owners/{ownerId}/edit", TEST_OWNER_ID).param("firstName",
	 * "Joe") .param("lastName", "Bloggs").param("city",
	 ...
	 * hasProperty("telephone", is("6085551023")))) .andExpect(model().attribute("owner",
	 * hasProperty("pets", not(empty())))) .andExpect(model().attribute("owner",
	 * hasProperty("pets", new BaseMatcher<List<Pet>>() {
	 *
